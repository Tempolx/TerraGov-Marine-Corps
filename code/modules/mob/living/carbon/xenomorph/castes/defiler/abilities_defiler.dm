...
/datum/action/ability/xeno_action/emit_neurogas/action_activate()
	...
	X.icon_state = "[X.xeno_caste.caste_name][(X.xeno_flags & XENO_ROUNY) ? " rouny" : ""][(X.xeno_flags & XENO_MAID) ? " maid" : ""]  Power Up"
	...
	if(!do_after(X, DEFILER_GAS_CHANNEL_TIME, NONE, null, BUSY_ICON_HOSTILE))
		if(!QDELETED(src))
			...
			X.icon_state = "[X.xeno_caste.caste_name][(X.xeno_flags & XENO_ROUNY) ? " rouny" : ""][(X.xeno_flags & XENO_MAID) ? " maid" : ""]  Running"
			return fail_activate()
	X.emitting_gas = FALSE
	X.icon_state = "[X.xeno_caste.caste_name][(X.xeno_flags & XENO_ROUNY) ? " rouny" : ""][(X.xeno_flags & XENO_MAID) ? " maid" : ""]  Running"
